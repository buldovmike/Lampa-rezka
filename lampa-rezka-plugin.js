/**
HDREZKA for Lampa/Luxo — v4.16.1 (fix: single-voice parsing, user_hash, silent playlist build)
*/
(function () {
'use strict';
...
function apiStream(card, voiceId, season, episode, cb, fail, silent) {
    ...
    getJsonAny(ajaxRel('ajax/get_cdn_series/'), form, card.rel, card._mirror, function (data) {
        ...
        if (!Object.keys(map).length) {
            var q0 = streamFromHtml(card);
            if (q0 && Object.keys(q0).length) { cb(q0); return; }
            if (!silent) fail(new Error('сервер не вернул ссылку'));
            return;
        }
        cb(map);
    }, function (e) {
...
function sectionTitle(t) { return $('' + esc(t) + ''); }
function cardEl(item, withProgress) {
    var metaLine = item.info || (item.year ? item.year : '');
    var el = $(' ' +
        '' + (item.poster ? '' : '') + ' ' +
        '' + esc(item.title || 'Без названия') + '' +
        '' + esc(metaLine) + (item.type === 'serial' ? ' · сериал' : '') + '' +
        '');
    if (withProgress) {
        var p = percentOf(item);
        if (p > 0) el.find('.rezka-card__poster').append('');
        if (item.episode) el.append(' S' + item.season + ' E' + item.episode + ' ');
    }
    return el;
}
...
function btnEl(label) { return $('' + esc(label) + ''); }
function actorEl(a, idx) {
    return $(' ' +
        '' + (a.img ? '' + esc((a.name || '?').charAt(0)) + '' : '') + '' +
        '' + esc(a.name) + '' +
        '' + esc(a.role || 'Фильмография') + '');
}
function frCardEl(f) {
...
function RezkaMain(object) {
    ...
    function fillContinue(grid) {
        getText('continue/', { method: 'GET' }, function (html) {
            if (!inited) return;
            grid.empty();
            if (isBadHtml(html)) { grid.append('«Досмотреть» недоступно (нужна авторизация rezka)'); nav(); return; }
            var items = parseContinue(html).slice(0, 15);
            if (!items.length) { grid.append('На rezka нет начатых просмотров'); nav(); return; }
            ...
            if (inited) { grid.html('«Досмотреть» недоступно (нужна авторизация rezka)'); nav(); }
        });
    }
    function build() {
        scroll.clear();
        ...
        if (!jarHasAuth()) {
            scroll.append($('Нет авторизации rezka.fi: Настройки → HDREZKA → «Cookies вручную» или «Войти».'));
        }
        nav();
    }
...
function RezkaCard(object) {
    ...
    function applyActorImg(a, idx) {
        try {
            ...
            if (ph.length && !ph.find('img').length) ph.html('');
        } catch (e) {}
    }
    ...
    function chipsHtml() {
        var c = [];
        if (card.year) c.push('' + esc(card.year) + '');
        if (card.country) c.push('' + esc(card.country) + '');
        if (card.quality) c.push('' + esc(card.quality) + '');
        if (card.duration) c.push('' + esc(card.duration) + '');
        if (card.age) c.push('' + esc(card.age) + '');
        if (card.imdb) c.push(' IMDb ' + esc(card.imdb) + '');
        if (card.kp) c.push(' KP ' + esc(card.kp) + '');
        (card.genres || []).slice(0, 3).forEach(function (g) { c.push('' + esc(g) + ''); });
        return c.join('');
    }
    function render() {
        scroll.clear();
        ...
        scroll.append($(' ' +
            ...
            '' +
            '' + esc(card.title) + '' +
            (card.origTitle ? '' + esc(card.origTitle) + '' : '') +
            '' + chipsHtml() + '' +
            '' + esc(card.descr) + '' +
            ''));
        if (card._htmlFail) scroll.append($('HTML карточки не получен: ' + esc(snippet(card._htmlFail, 120)) + ''));
        var btns = $('');
