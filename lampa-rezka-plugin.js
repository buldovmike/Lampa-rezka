...
// === REZKA DIRECT PLAYER CORE ===
var rezkaDirectState = {
    close: null
};

function rezkaDirectClose() {
    if (rezkaDirectState.close) {
        try {
            rezkaDirectState.close();
        } catch (e) {}
...
function rezkaDirectEnsureCss() {
    var oldIds = ['rezka-direct-css', 'rezka-direct-css-v2', 'rezka-direct-css-v3', 'rezka-direct-css-v4', 'rezka-direct-css-v5', 'rezka-direct-css-v6'];
    for (var oi = 0; oi < oldIds.length; oi++) {
        var oldEl = document.getElementById(oldIds[oi]);
        if (oldEl && oldEl.parentNode) oldEl.parentNode.removeChild(oldEl);
    }
    if (!document.getElementById('rezka-direct-css-v9')) {
        var css9 = '' +
            '.rd-modal__box--side .rd-modal__list{padding:2px 8px 14px;}' +
            '.rd-modal__list .rd-modal__item--ep.focus{background:rgba(255,255,255,.14);box-shadow:inset 0 0 0 3px #fff;-webkit-transform:none;transform:none;}' +
            '.rd-modal__list .rd-modal__item--ep.current{background:rgba(255,255,255,.07);box-shadow:inset 3px 0 0 #5eead4;}' +
            '.rd-modal__list .rd-modal__item--ep.current.focus{background:rgba(255,255,255,.14);box-shadow:inset 0 0 0 3px #fff,inset 3px 0 0 #5eead4;}' +
            '.rd-ep__title--plain{color:rgba(255,255,255,.85);}' +
            '.rd-modal__box--side .rd-modal__title{margin-bottom:8px;}' +
            '.rd-modal__count{margin:0 0 14px;font-size:20px;font-weight:500;color:rgba(255,255,255,.55);}';
        var st9 = document.createElement('style');
        st9.id = 'rezka-direct-css-v9';
        try { st9.appendChild(document.createTextNode(css9)); } catch (e9) { st9.text = css9; }
    ...
    var css = '' +
        ...
        '.rd-btn--play{width:88px;height:88px;padding:0;border-radius:50%;background:linear-gradient(135deg,#14b8a6,#5eead4);box-shadow:0 12px 40px rgba(20,184,166,.40);}' +
        '.rd-btn--play svg{width:42px;height:42px;}' +
        '.rd-btn--play.focus{box-shadow:0 0 0 4px #fff,0 12px 44px rgba(20,184,166,.50);}' +
        ...
        '.rd-fill{height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg,#14b8a6,#67e8f9);}' +
        ...
        '.rd-modal.rd-modal--side{-webkit-justify-content:flex-end;justify-content:flex-end;background:linear-gradient(90deg,rgba(0,0,0,0) 0%,rgba(0,0,0,.55) 35%);}' +
        '.rd-modal__box.rd-modal__box--side{width:40vw;min-width:420px;max-width:560px;height:88vh;max-height:88vh;display:-webkit-flex;display:flex;-webkit-flex-direction:column;flex-direction:column;border-radius:28px 0 0 28px;padding:30px 30px 26px;overflow:hidden;}' +
        '.rd-modal__list{position:relative;-webkit-flex:1;flex:1;overflow-y:auto;overflow-x:hidden;margin-top:6px;-webkit-overflow-scrolling:touch;}' +
        '.rd-modal__list::-webkit-scrollbar{width:0;height:0;}' +
        '.rd-modal__item--ep{padding:14px 22px 12px;margin-bottom:10px;border-radius:18px;background:rgba(255,255,255,.06);}' +
        '.rd-modal__item--ep.focus{background:rgba(255,255,255,.16);box-shadow:0 0 0 3px #fff,0 10px 30px rgba(0,0,0,.45);-webkit-transform:scale(1.02);transform:scale(1.02);}' +
        '.rd-ep__main{display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;gap:14px;min-width:0;}' +
        '.rd-ep__num{-webkit-flex-shrink:0;flex-shrink:0;min-width:1.4em;text-align:right;font-size:22px;font-weight:700;color:rgba(255,255,255,.42);font-variant-numeric:tabular-nums;}' +
        '.rd-ep__title{min-width:0;font-size:26px;font-weight:600;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}' +
        '.rd-modal__item--ep.current .rd-ep__num,.rd-modal__item--ep.current .rd-ep__title{color:#5eead4;}' +
        '.rd-modal__itemsub{margin-top:6px;font-size:20px;font-weight:500;color:rgba(255,255,255,.60);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}' +
        '.rd-ep__bar{margin-top:10px;height:4px;border-radius:999px;background:rgba(255,255,255,.16);overflow:hidden;}' +
        '.rd-ep__fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#14b8a6,#67e8f9);}' +
...
function rezkaWatchStart(meta) {
    ...
    function send() {
        if (!meta || !meta._card || !meta._card.contentId) return;

        var form = {
            action: 'add',
            id: meta._card.contentId
        };

        if (meta.voice_id) form.translator_id = meta.voice_id;
        if (meta.season) form.season = meta.season;
        if (meta.episode) form.episode = meta.episode;
...
function rezkaDirectPlay(url, meta) {
    ...
    rezkaDirectClose();
    rezkaDirectEnsureCss();

    var hash = meta && meta.hash ? meta.hash : '';
    var savedTime = rezkaTimelineGetTime(hash);
    var pendingSeek = savedTime > 0 ? savedTime : 0;
    var initialAsk = pendingSeek > 90;
    var resumeDecided = !initialAsk;
    ...
    function showControls(autoHide) {
        ...
        if (autoHide && started && !v.paused && !loading && !modalOpen && !confirmOpen && !resumeOpen) {
    ...
    function maybeAutoHide() {
        ...
        if (uiTimer) { clearTimeout(uiTimer); uiTimer = null; }
        if (started && !v.paused && !loading && !modalOpen && !confirmOpen && !resumeOpen) {
            uiTimer = setTimeout(hideControls, 4000);
        }
    }
    function hideControls() {
        if (closed || !v || v.paused || loading || modalOpen || confirmOpen || resumeOpen) return;
    ...
    function wheelHandler(e) {
        if (closed || modalOpen || confirmOpen || resumeOpen) return;
        var dx = 0;
        try {
    ...
    function openResume() {
        if (closed || resumeOpen) return;
        resumeOpen = true;
        resumeIdx = 0;
        $resume = $('<div class="rd-modal"><div class="rd-modal__box rd-modal__box--confirm"><div class="rd-modal__title rd-modal__title--confirm">Продолжить с ' + fmt(pendingSeek) + '?</div><div class="rd-modal__row"></div></div></div>');
        var $row = $resume.find('.rd-modal__row');
        $row.append($('<div class="rd-modal__item"></div>').text('Продолжить'));
        $row.append($('<div class="rd-modal__item"></div>').text('Начать сначала'));
        $wrap.append($resume);
        renderResume();
    }
    function renderResume() {
        if (!$resume) return;
        var items = $resume.find('.rd-modal__item');
        items.removeClass('focus');
        items.eq(resumeIdx).addClass('focus');
    }
    ...
    function openEpisodes() {
        ...
        modalOpen = true;
        $modal = $('<div class="rd-modal rd-modal--side"><div class="rd-modal__box rd-modal__box--side"><div class="rd-modal__title">Серии</div><div class="rd-modal__count"></div><div class="rd-modal__list"></div></div></div>');
        $modal.find('.rd-modal__count').text((meta.season ? 'Сезон ' + meta.season : 'Сезон не указан') + ' · ' + modalItems.length + ' сер.');
        var $list = $modal.find('.rd-modal__list');
    ...
    function selectEdgeActivate() {
        ...
        lastSelectAct = now;
        if (resumeOpen) { resumeApply(); return; }
    ...
    function keyHandler(e) {
        ...
        if (resumeOpen) { handleResumeKey(e); return; }
...
function mergeSchedule(card, sd) {
    if (!sd || !sd.episodes) return sd;
    var sched = scrapeScheduleFromHtml((card && card._html) || '');
    if (!Object.keys(sched).length) return sd;
    for (var sid in sd.episodes) {
        for (var i = 0; i < sd.episodes[sid].length; i++) {
...
function apiSeasonData(card, voiceId, cb, fail) {
    var _cbRaw = cb;
    cb = function (sd) { _cbRaw(mergeSchedule(card, sd)); };
    card._sd = card._sd || {};
...
function playMeta(meta) {
    ...
    apiStream(meta._card, meta.voice_id, meta.season, meta.episode, function (quality) {
        ...
        playMetaWithQuality(meta, quality);
    }, function (e) {
        Lampa.Loading.stop();
